...
namespace NBADATA.Pages
{
    public class CompareModel : PageModel
    {
        ...
        private string? FindPhotoUrl(Player? p)
        {
            ...
            // Buscar en la carpeta player_images dentro del proyecto
            var projectRoot = _env.ContentRootPath ?? Directory.GetCurrentDirectory();
            var playerImagesDir = Path.Combine(projectRoot, "player_images");
            
            // Normalizar el nombre del jugador al formato "Nombre-Apellido"
            string FormatPlayerName(string fullName)
            {
                ...
                // Formato: "Nombre-Apellido" o "Nombre1-Nombre2-Apellido"
                return string.Join("-", words);
            }
            
            // Generar nombres de archivo candidatos
            var formattedName = FormatPlayerName(p.FullName);
            var candidates = new List<string>();
            
            if (!string.IsNullOrWhiteSpace(formattedName))
            {
                // Primera letra mayúscula en cada palabra (formato más común: "Anthony-Davis.jpg")
                var titleCase = System.Globalization.CultureInfo.CurrentCulture.TextInfo.ToTitleCase(formattedName.ToLowerInvariant());
                candidates.Add($"{titleCase}.jpg");
                candidates.Add($"{titleCase}.JPG");
                
                // Formato exacto como viene del nombre: "Nombre-Apellido.jpg"
                candidates.Add($"{formattedName}.jpg");
                candidates.Add($"{formattedName}.JPG");
                
                // Variaciones con mayúsculas/minúsculas
                candidates.Add($"{formattedName.ToLowerInvariant()}.jpg");
                candidates.Add($"{formattedName.ToLowerInvariant()}.JPG");
            }
            
            // También buscar por ID como fallback
            candidates.Add($"{p.Id}.jpg");
            candidates.Add($"{p.Id}.JPG");

            // Buscar archivos en el directorio (búsqueda case-insensitive)
            if (Directory.Exists(playerImagesDir))
            {
                // Obtener todos los archivos .jpg y .JPG
                var allFiles = Directory.GetFiles(playerImagesDir, "*.*", SearchOption.TopDirectoryOnly)
                    .Where(f => f.EndsWith(".jpg", StringComparison.OrdinalIgnoreCase))
                    .Select(f => Path.GetFileName(f))
                    .ToList();

                // Primero intentar coincidencia exacta con los candidatos (case-insensitive)
                foreach (var candidate in candidates)
                {
                    var found = allFiles.FirstOrDefault(f => 
                        string.Equals(f, candidate, StringComparison.OrdinalIgnoreCase));
                    if (found != null)
                    {
                        return $"/player_images/{found}";
                    }
                }

                // Si no hay coincidencia exacta, buscar por nombre del jugador (búsqueda parcial)
                if (!string.IsNullOrWhiteSpace(formattedName))
                {
                    var nameParts = formattedName.Split('-', StringSplitOptions.RemoveEmptyEntries)
                        .Where(part => part.Length > 1) // Ignorar partes muy cortas
                        .ToList();
                    ...
                    if (nameParts.Count > 0)
                    {
                        var found = allFiles.FirstOrDefault(f =>
                        {
                            ...
                            // Verificar que todas las partes importantes del nombre estén en el archivo
                            return nameParts.All(part => 
                                fileNameWithoutExt.Contains(part, StringComparison.OrdinalIgnoreCase));
                        });
